...
The **MY** component adult mosquito ecology and infection dynamics. We
let **M** describe a state space describing mosquito ecology, and **Y**
an embedded state space describing parasite or pathogen infection
dynamics. Since **Y** is embedded within **M,** the two processes are
handled together.
...
Each **MY** module includes XX required functions and some optional
ones.
...
repository
[human-MY-SI.R](https://github.com/dd-harp/ramp.xds/blob/main/R/human-MY-SIS.R).
...
1.  The dynamics are defined by at least one of the following is
    ...
    - `dMYdt.MYname` :: differential equations are defined by a function
      that computes the derivatives. In `ramp.xds` these are encoded in
      a function called `dMYdt.` The function is set up to be solved by
      [`deSolve::ode`](https://rdrr.io/pkg/deSolve/man/ode.html) or
      `deSolve::dede.`

    - `Update_MYt.MYname` :: discrete time systems are defined by the
      function that updates the state variables in one time step. In
      `ramp.xds` these are encoded in a function called `Update_MYt`
...
2.  `make_MY_obj_MYname` :: returns a structured list called an **MY**
    model object:
...
3.  `setup_MY_obj.MYname` is a wrapper that calls `make_MY_obj_MYname`
    and (for the \\i^{th}\\ species) attaches the object as
    `xds_obj$MY_obj[[i]]`

### Parameters

4.  `change_MY_pars.MYname` changes the values of some parameters. It is
    designed to be used after setup. New parameter values are passed by
    name in a list called `options.`

5.  `get_MY_pars.MYname` is a utility to inspect the values of the
    parameters.
...
6.  `setup_MY_ix.MYname` - is the function that assigns an index to each
    variable in the model, and stores it as `xds_obj$MY_obj[[i]]$ix.`
    The indices are returned as a named list.

7.  `get_MY_vars.MYname` - retrieves the value of variables from the
    state variables vector \\y\\ at a point in time and returns the
    values by name in a list; the function gets called by `dMYdt` and by
    `change_MY_inits` and it can be useful in other contexts.

8.  `parse_MY_orbits.MYname` - this function is like `get_MY_vars` but
    it parses the matrix of outputs returned by `xds_solve.`
...
9.  `make_MY_inits_MYname` - each model must include a function that
...
10. `setup_MY_inits.MYname` - is a wrapper, that gets called by
    `xds_setup` and that calls `make_MY_inits_MYname.` The setup
    `options` are passed to overwrite default values. The initial values
    are stored as `MY_obj$inits.`
...
16. `F_ni.MYname` - compute net infectiousness (NI) for each stratum. If
...
18. `skill_set_MY.MYname` :: describes model capabilities and
    limitations

19. `check_MY.MYname` :: at the end of `xds_setup` and at the beginning

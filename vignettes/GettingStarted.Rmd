...
**`ramp.xds`** reduces the costs of building, solving, and analyzing models describing malaria transmission dynamics and control. It can also be used to study the dynamics and control of other mosquito-transmitted pathogens. This vignette gives a basic overview.  Closely related vignettes are:

+ [Working with Models in **`ramp.xds`**](Working.html) - a primer on learning how to **`ramp.xds`** 

+ [Modularity](modularity.html) - an overview of the modular design.  

Also, browse the menus of the [**`ramp.xds`** homepage](https://dd-harp.github.io/ramp.xds/index.html)
...
When `xde_solve` is called, the **`xds` model object** it returns has replaced any old values. Pulling information is easy, but a user will probably have to write a function to do it.  
...
```
Another set of functions plots standard outputs:

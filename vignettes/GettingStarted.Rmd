...
The default for the aquatic mosquito dynamical component is the `trivial` module, where the `mean` is called `Lambda`.

It is easy to change the `trace` function. First, we define `F_season` and change the mean value of `Lambda.` To set up a seasonality function, we pass a parameter set from `makepar_F_sin`: 

...
```{r}
out1 <- xde_solve_cohort(eir, 0, params)$outputs$cohort
```
...
clrs = turbo(5)
with(out1,{
  plot(age, XH[[1]]$m1[,1], col = clrs[1], ylim = range(XH[[1]]$m1), type = "l")
  lines(age, XH[[1]]$m1[,2], col = clrs[2])
  lines(age, XH[[1]]$m1[,3], col =clrs[5])
  
  lines(age, XH[[1]]$m2[,1], col = clrs[1], lty = 2)
  lines(age, XH[[1]]$m2[,2], col = clrs[2], lty=2)
  lines(age, XH[[1]]$m2[,3], col =clrs[5], lty=2)
})
```
...
```{r}
xds_setup_cohort(eir, bday=0, Xname="hMoI", HPop=H, Xopts = Xo) ->test_hMoI_xde
```

```{r}
xde_solve_cohort(eir, 0, test_hMoI_xde)$outputs$cohort -> out2

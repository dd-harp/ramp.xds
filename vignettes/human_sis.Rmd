...
Xname = "SIS"
```
For the **XH** component, the **SIS** module implements a basic system of differential equations, that is often called the **SIS** compartment model (see Figure 1). This vignette describes the model and its implementation in **`ramp.xds.`**
For a more mathematical introduction, see [**Malaria Theory** -- SIS Dynamics](https://faculty.washington.edu/smitdave/malaria_theory/sis.html){target="_blank"}
...
$$
Once again, since $H=S+I,$ one of these equations is redundant. We compute $dH/dt$ and $dI/dt.$
...
Users interested in more technical details should read [our fully worked multi-patch example](ex_534.html). 
...
Or we can use the `get_XH_orbits` function to get the values of the orbits. This gets the return values and pulls of the the values of $I$ at the last time step: 

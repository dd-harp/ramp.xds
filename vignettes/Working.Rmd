...
The functions `xde_solve` and `dts_solve` can be called to solve dynamical systems. In these systems, *time* is the independent variable. Given a set of initial values, `xde_solve` or `dts_solve` compute the values of the dependent variables at those time points. Other functions return stable orbits for models with a canonical seasonal signal.  
...
A set of utilities was developed to work with initial values, making it easy to change the intitial values through a function call. In solving, `xde_solve` calls `get_inits` with a `flatten=TRUE` 

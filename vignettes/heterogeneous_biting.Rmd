...
In `ramp.xde,` the term **heterogeneous biting** is used to describe differences among population strata in the expected rate of exposure to infective mosquitoes. 
...
**Heterogeneous Biting** is defined throughout the `ramp.xde` implementation and documentation as a difference in the relative biting rates for two strata that are otherwise identical. The implementation relies on two concepts:
...
In `ramp.xde,` the concept of human *availability* for blood feeding is also modified by time spent, and the algorithms were designed to deal with both changing denominators and search weights. In a patch, the available human population is a measure of the search weighted ambient population density.

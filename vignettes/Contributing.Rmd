...
Thank you for taking the time to contribute to **SimBA** (Extensible Differential Equations for mosquito-borne pathogen modeling). In this document we will explain the following topics:
...
Our workflow is designed around branches. The `main`, `dev`, and `gh-pages` branches always exist in the repository. Other branches exist as long as the person(s) working on them need them, and should be deleted after they have been merged into `dev`.
  ...
  * `gh-pages`: this branch exists to support our GitHub pages website (the one you are browsing right now). It is updated automatically by our [pkgdown continuous integration workflow](https://github.com/dd-harp/SimBA/blob/main/.github/workflows/pkgdown.yaml) which rebuilds the website when `main` is pushed to (i.e. when a PR is merged into `main`). `gh-pages` should be considered a "read-only" branch, even for the core dev team.
...
Let's say you have a great idea for a new feature, which is a change to improve this document. Here's how you would get started with our Git workflow to get your feature on the road to being merged into the **SimBA** software.
...
Please note, we assume some familiarity with Git commands here. For a reference on the meaning of these commands, please see the [Git documentation](https://git-scm.com/docs).
  ...
  1. To start to work on **SimBA**, you should first clone the repository. In a terminal window, navigate to the directory you want the `exDE` folder to appear in, and then run `git clone git@github.com:dd-harp/exDE.git`, if you use SSH keys, or `git clone https://github.com/dd-harp/exDE.git` to clone with HTTPS. These links can be found if you click the green "Code" button from the [main repository page](https://github.com/dd-harp/exDE).
  2. Once you clone the repository, go inside it with `cd SimBA`. Then run `git fetch --all` which retrieves information about all the branches on the remote (the repository on the GitHub website) into your local repository (the one on your computer).
  ...
  5. To push this new branch to the remote repository, run `git push origin feat/UpdateContributing`. It will now appear in the [list of branches](https://github.com/dd-harp/SimBA/branches).
  6. Now, make your changes! We recommend editing the package in [RStudio](https://posit.co/products/open-source/rstudio/) using the R project file `SimBA.Rproj` in the repository. This will be explained more in Section [3](#sec3).
...
Some files in the top level directory that are important to be aware of are:
  ...
  * `NAMESPACE`: a *read-only* file produced when running [roxygen2](https://roxygen2.r-lib.org/) to build documentation for the package, it tells R what functions are exported by this package into a user's R session when the load **SimBA** with `library(exDE)`. It should never be modified by hand.
  * `NEWS.md`: the NEWS page on the package's [CRAN page](https://cran.r-project.org/package=SimBA) as well as the News tab on the [GitHub pages website](https://dd-harp.github.io/exDE/) are built from this file; it should only be modified by the dev team when ready to update `main` with a new version. For more information, see the `utils::news` help file.
  * `README.md`: this is the main readme file which is displayed on the GitHub repository, the main page of the [GitHub pages website](https://dd-harp.github.io/SimBA/), and the README page on the package's [CRAN page](https://cran.r-project.org/package=exDE).
...
The rest of the package is organized in directories. For more detail, we recommend reading the guides linked to earlier.
  ...
  * `vignettes/`: this folder contains .Rmd files contains the vignettes of the R package, longer form articles that document specific functionality in **SimBA**. They are available on the [GitHub pages website](https://dd-harp.github.io/exDE/) via the top drop down menus and the package's [CRAN page](https://cran.r-project.org/package=exDE).
...
Each model component will have a "base" file ending in `-interface` that defines the generic interface which any model for that component must implement. For example `human-interface.R` defines the generic methods any human disease model must implement and `human-SIS.R` defines specific methods for the SIS model of human disease dynamics.
  ...
  * `diffeqn.R`: the **SimBA** generalized differential equations which implement the framework; these should not typically be modified.
...
If you are requested to be a reviewer, it's your responsibility to:
  ...
  * Evaluate whether or not the proposed addition is within the scope of the software (e.g. directly-transmitted diseases are outside the scope of **SimBA**).
  ...
  * If the PR makes significant modifications to **SimBA**, work with the author (if they have not already done so) until the PR has tests and vignettes to document the new functions, and that those are added to the `_pkgdown.yml` file so the new features appear on the website.
  * While in general, **SimBA**'s design philosophy is that elegant and clear design that mirrors the mathematical description of the model is to be preferred over fast code in cases where there is a decision to be made, please correct obvious inefficiencies (e.g. be aware of matrix operations and try to structure parentheses to minimize computational cost).
...
In **SimBA** we use [GitHub Actions](https://docs.github.com/en/actions) to automatically run continuous integration workflows, which are automated scripts which run when triggered by certain events (i.e. PRs or pushing to certain branches). The 3 workflows we run can be found in `.github/workflows`:
  ...
  * `pkgdown.yaml`: this runs `pkgdown::build_site_github_pages` and pushes the new website to the `gh-pages` branch. It runs on pushes to `main` (i.e. when a PR is merged into `main`) and when a new [GitHub release](https://github.com/dd-harp/SimBA/releases) is made.
  * `test-coverage.yaml`: this runs `covr::codecov` which updates **SimBA**'s [codecov](https://app.codecov.io/gh/dd-harp/exDE) integration. It runs on pushes to `main`.
...
These workflows may encounter errors. Do not panic when this happens. Errors can be investigated by clicking on the failing workflow in the [Actions](https://github.com/dd-harp/SimBA/actions) tab, and then by clicking on the specific failing job(s) on the left hand side of the screen under "Jobs". The errors will then be displayed. The [Github Actions documentation](https://docs.github.com/en/actions/monitoring-and-troubleshooting-workflows/using-workflow-run-logs) also describes how to diagnose errors.

Errors in `R-CMD-check` stemming from tests may sometimes be due to numerical errors; because **SimBA** can only solve the model up to floating point errors, sometimes false positive errors are reported when the numerical routines encounter problems. Restart the tests by clicking "Re-run jobs" in the upper right hand and see if the errors persist. Errors in `pkgdown` are most likely cause by an ill-formed `_pkgdown.yml` file, please consult the [pkgdown documentation](https://pkgdown.r-lib.org/articles/pkgdown.html) or post an issue to get more help.
...
You are now ready to create a PR from `dev` to `main`. After following the standard procedure (Section [5](#sec5)) with the exception of deleting `dev` (protected from deletion), you are ready to update the [CRAN version](https://cran.r-project.org/package=SimBA) of **exDE**.

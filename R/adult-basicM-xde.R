...
#' @return a named [list]
#' @export
MBionomics.basicM_xde <- function(t, y, pars, s) {
...
#' @export
F_fqZ.basicM_xde <- function(t, y, pars, s) {
...
#' @return a [numeric] vector of length `nPatches`
#' @export
F_fqM.basicM_xde <- function(t, y, pars, s) {
...
#' @return a [numeric] vector of length `nPatches`
#' @export
F_eggs.basicM_xde <- function(t, y, pars, s) {
...
#' @export
dMYZdt.basicM_xde <- function(t, y, pars, s){
...
#' @title Make parameters for basicM_xde ODE adult mosquito model
...
#' @title Setup the basicM_xde model
#' @description Implements [setup_MYZinits] for the basicM_xde model
#' @inheritParams setup_MYZinits
#' @return a [list] vector
#' @export
setup_MYZinits.basicM_xde = function(pars, s, MYZopts=list()){
  pars$MYZinits[[s]] = make_MYZinits_basicM_xde(pars$nPatches, MYZopts)
  return(pars)
}



#' @title Make inits for basicM_xde adult mosquito model
...
#' @return none
#' @export
make_MYZinits_basicM_xde = function(nPatches, MYZopts = list(),
                            M0=5, P0=1){
  with(MYZopts,{
...
#' @return none
#' @export
update_inits_MYZ.basicM_xde <- function(pars, y0, s) {with(pars$ix$MYZ[[s]],{
  M = y0[M_ix]
  P = y0[P_ix]
  pars$MYZinits[[s]] = make_MYZinits_basicM_xde(pars, list(), M0=M, P0=P)
  return(pars)
})}
...
#' @export
make_indices_MYZ.basicM_xde <- function(pars, s) {with(pars,{
...
#' @return none
#' @export
make_inits_MYZ_basicM_xde <- function(pars, M0, P0) {
  pars$MYZinits[[1]] = list(M=M0, P=P0)
  return(pars)
}

#' @title Return initial values as a vector
#' @description Implements [get_inits_MYZ] for the basicM_xde model.
#' @inheritParams get_inits_MYZ
#' @return none
#' @export
get_inits_MYZ.basicM_xde <- function(pars, s) {with(pars$MYZinits[[s]],{
  c(M, P)
})}

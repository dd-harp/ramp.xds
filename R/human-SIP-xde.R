# specialized methods for the human SIP_xde model

#' @title Size of effective infectious human population
#' @description Implements [F_X] for the SIP_xde model.
#' @inheritParams F_X
#' @return a [numeric] vector of length `nStrata`
#' @export
F_X.SIP_xde <- function(t, y, pars, i) {
...
#' @return a [numeric] vector of length `nStrata`
#' @export
F_H.SIP_xde <- function(t, y, pars, i){
  with(pars$ix$X[[i]],{
    S = y[S_ix]
    I = y[I_ix]
    P = y[P_ix]
...
#' @description Implements [F_pr] for the SIP_xde model.
#' @inheritParams F_pr
#' @return a [numeric] vector of length `nStrata`
#' @export
F_pr.SIP_xde <- function(varslist, pars, i) {
  pr = with(varslist$XH[[i]], I/H)
  return(pr)
}

#' @title Infection blocking pre-erythrocytic immunity
#' @description Implements [F_b] for the SIP_xde model.
#' @inheritParams F_b
#' @return a [numeric] vector of length `nStrata`
#' @export
F_b.SIP_xde <- function(y, pars,i) {
  with(pars$Xpar[[i]], b)
}

#' @title Derivatives for human population
#' @description Implements [dXdt] for the SIP_xde model.
#' @inheritParams dXdt
#' @return a [numeric] vector
#' @export
dXdt.SIP_xde <- function(t, y, pars, i){
...
#' @return a [list]
#' @export
list_Xvars.SIP_xde <- function(y, pars, i) {
  with(pars$ix$X[[i]],
      return(list(
        S = y[S_ix],
        I = y[I_ix],
...
#' @title Compute the HTC for the SIP_xde model
#' @description Implements [HTC] for the SIP_xde model with demography.
#' @inheritParams HTC
#' @return a [numeric] vector
#' @export
HTC.SIP_xde <- function(pars, i) {
  with(pars$Xpar[[i]],
...
#' @title Setup Xinits.SIP_xde
#' @description Implements [setup_Xinits] for the SIP_xde model
#' @inheritParams setup_Xinits
#' @return a [list] vector
#' @export
setup_Xinits.SIP_xde = function(pars, i, Xopts=list()){
  pars$Xinits[[i]] = make_Xinits_SIP_xde(pars$Hpar[[i]]$nStrata, Xopts, H0=pars$Hpar[[i]]$H)
  return(pars)
}

#' @title Make parameters for SIP_xde human model, with defaults
...
#' @return a [list]
#' @export
make_Xpar_SIP_xde = function(nStrata, Xopts=list(),
                         b=0.55, r=1/180, c=0.15,
                         rho=.1, eta=1/25, xi=1/365){
  with(Xopts,{
    Xpar = list()
    class(Xpar) <- c("SIP_xde")
...
#' @title Make initial values for the SIP_xde human model, with defaults
#' @param nStrata the number of population strata
#' @param Xopts a [list] that could overwrite defaults
...
#' @return a [list]
#' @export
make_Xinits_SIP_xde = function(nStrata, Xopts = list(), H0=NULL, S0=NULL,
...
#' @title Parse the output of deSolve and return variables for the SIP_xde model
#' @description Implements [parse_outputs_X] for the SIP_xde model
#' @inheritParams parse_outputs_X
#' @return none
#' @export
parse_outputs_X.SIP_xde <- function(outputs, pars, i) {
  time = outputs[,1]
  with(pars$ix$X[[i]],{
    S = outputs[,S_ix+1]
    I = outputs[,I_ix+1]
...
#' @description Implements [make_indices_X] for the SIP_xde model.
#' @inheritParams make_indices_X
#' @return none
#' @importFrom utils tail
#' @export
make_indices_X.SIP_xde <- function(pars, i) {with(pars,{
...
#' @title Make parameters for SIP_xde human model
#' @param pars a [list]
#' @param b transmission probability (efficiency) from mosquito to human
#' @param c transmission probability (efficiency) from human to mosquito
#' @param r recovery rate
...
#' @return none
#' @export
make_parameters_X_SIP_xde <- function(pars, b, c, r, rho, eta, xi){
...
#' @title Make inits for SIP_xde human model
#' @param pars a [list]
...
#' @return none
#' @export
make_inits_X_SIP_xde <- function(pars, S0, I0, P0) {
  stopifnot(is.numeric(I0), is.numeric(P0), is.numeric(S0))
  pars$Xinits[[1]] = list(S=S0, I=I0, P=P0)
  return(pars)
}

#' @title Update inits for the SIP_xde human model from a vector of states
#' @inheritParams update_inits_X
#' @return none
#' @export
update_inits_X.SIP_xde <- function(pars, y0, i) {
  with(list_Xvars(y0, pars, i),{
  pars$Xinits[[i]] = make_Xinits_SIP_xde(pars$Hpar[[i]]$nStrata, list(), S0=S, I0=I, P0=P)
  return(pars)
})}
...
#' @return none
#' @export
get_inits_X.SIP_xde <- function(pars, i){with(pars$Xinits[[i]],{
  c(S, I, P)
})}


#' Plot the density of infected individuals for the SIP_xde model
#'
#' @inheritParams xds_plot_X
#' @export
xds_plot_X.SIP_xde = function(pars, i=1, clrs=c("darkblue", "darkred", "darkgreen"), llty=1, stable=FALSE, add_axes=TRUE){
  ...
  add_lines_X_SIP_xde(vars$XH[[i]], pars, clrs, llty)
}


#' Add lines for the density of infected individuals for the SIP_xde model
#'
#' @param XH a list with the outputs of parse_outputs_X.SIP_xde
#' @param pars a list that defines an `ramp.xde` model (*e.g.*,  generated by `xde_setup()`)
#' @param clrs a vector of colors
#' @param llty an integer (or integers) to set the `lty` for plotting
#'
#' @export
add_lines_X_SIP_xde = function(XH, pars, clrs=c("darkblue", "darkred", "darkgreen"), llty=1){
  with(XH,{

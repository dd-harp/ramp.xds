...
#' @description Implements [parse_deout_X] for the SIP_xde model
#' @inheritParams parse_deout_X
#' @return none
#' @export
parse_deout_X.SIP_xde <- function(deout, pars, i) {
  time = deout[,1]
  with(pars$ix$X[[i]],{
    S = deout[,S_ix+1]
    I = deout[,I_ix+1]
...
#' @export
xde_plot_X.SIP_xde = function(pars, i=1, clrs=c("darkblue", "darkred", "darkgreen"), llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xde_lines_X_SIP_xde(vars$XH[[i]], pars, clrs, llty)
}


#' Add lines for the density of infected individuals for the SIP_xde model
#'
#' @param XH a list with the outputs of parse_deout_X.SIP_xde
#' @param pars a list that defines an `ramp.xde` model (*e.g.*,  generated by `xde_setup()`)
#' @param clrs a vector of colors
#' @param llty an integer (or integers) to set the `lty` for plotting
#'
#' @export
xde_lines_X_SIP_xde = function(XH, pars, clrs=c("darkblue", "darkred", "darkgreen"), llty=1){
  with(XH,{

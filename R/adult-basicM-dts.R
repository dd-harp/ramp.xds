# specialized methods for the adult mosquito basicM_dts model

#' @title Reset bloodfeeding and mortality rates to baseline
#' @description Implements [MBionomics] for the basicM_dts model
#' @inheritParams MBionomics
#' @return the model as a [list]
#' @export
MBionomics.basicM_dts <- function(t, y, pars, s) {
  with(pars$MYZpar[[s]],{
...
#' @description Implements [F_fqZ] for the basicM_dts model.
#' @inheritParams F_fqZ
#' @return a [numeric] vector of length `nPatches`
#' @export
F_fqZ.basicM_dts <- function(t, y, pars, s) {
  numeric(0)
}

#' @title The net blood feeding rate of the infective mosquito population in a patch
#' @description Implements [F_fqM] for the basicM_dts model.
#' @inheritParams F_fqM
#' @return a [numeric] vector of length `nPatches`
#' @export
F_fqM.basicM_dts <- function(t, y, pars, s) {
  with(pars$MYZpar[[s]], f*q)*y[pars$ix$MYZ[[s]]$M_ix]
}

#' @title Number of eggs laid by adult mosquitoes
#' @description Implements [F_eggs] for the basicM_dts model.
#' @inheritParams F_eggs
#' @return a [numeric] vector of length `nPatches`
#' @export
F_eggs.basicM_dts <- function(t, y, pars, s) {
...
#' @title Make parameters for basicM_dts adult mosquito model
...
#' @title Setup initial values for the basicM_dts model
#' @description Implements [setup_MYZinits] for the basicM_dts model
#' @inheritParams setup_MYZinits
#' @return a [list]
#' @export
setup_MYZinits.basicM_dts = function(pars, s, MYZopts=list()){
  pars$MYZinits[[s]] = with(pars$MYZpar[[s]], make_MYZinits_basicM_dts(nPatches, MYZopts))
  return(pars)
}

#' @title Make inits for basicM_dts adult mosquito model
...
#' @return a [list]
#' @export
make_MYZinits_basicM_dts = function(nPatches, MYZopts = list(),
                            M0=5, P0=1){
  with(MYZopts,{
...
#' @description Implements [make_indices_MYZ] for the basicM_dts model.
#' @inheritParams make_indices_MYZ
#' @return a [list]
#' @importFrom utils tail
#' @export
make_indices_MYZ.basicM_dts <- function(pars, s) {with(pars,{
...
#' @return a [list]
#' @export
list_MYZvars.basicM_dts <- function(y, pars, s){
  with(pars$ix$MYZ[[s]],
       return(list(
         M = y[M_ix],
         P = y[P_ix],
...
#' @return a [list]
#' @export
make_parameters_MYZ_basicM_dts <- function(pars,  p, sigma, f, q, nu, eggsPerBatch,  calK) {
...
#' @return a [list]
#' @export
make_inits_MYZ_basicM_dts <- function(pars, M0, P0) {
  pars$MYZinits = list()
  pars$MYZinits[[1]] = list(M=M0, P=P0)
  return(pars)
}

#' @title Parse the output of deSolve and return variables for the basicM_dts model
#' @description Implements [parse_outputs_MYZ] for the basicM_dts model
#' @inheritParams parse_outputs_MYZ
#' @return a [list]
#' @export
parse_outputs_MYZ.basicM_dts <- function(outputs, pars, s) {with(pars$ix$MYZ[[s]],{
  time = outputs[,1]
  M = outputs[,M_ix+1]
  P = outputs[,P_ix+1]
  parous = P/M
...
#' @description Implements [get_inits_MYZ] for the basicM_dts model.
#' @inheritParams get_inits_MYZ
#' @return [numeric]
#' @export
get_inits_MYZ.basicM_dts <- function(pars, s) {with(pars$MYZinits[[s]],{
  c(M, P)
})}
...
#' @return a [list]
#' @export
update_inits_MYZ.basicM_dts <- function(pars, y0, s) {with(pars$ix$MYZ[[s]],{
  M = y0[M_ix]
  P = y0[P_ix]
  pars$MYZinits[[s]] = make_MYZinits_basicM_dts(pars$nPatches,  list(), M0=M, P=P0)
  return(pars)
})}

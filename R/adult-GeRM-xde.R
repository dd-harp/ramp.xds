# specialized methods for the adult mosquito GeRM_xde model

#' @title Reset bloodfeeding and mortality rates to baseline
#' @description Implements [MBionomics] for the GeRM_xde model
#' @inheritParams MBionomics
#' @return the model as a [list]
#' @export
MBionomics.GeRM_xde <- function(t, y, pars, s) {
  with(pars,{
...
#' @description Implements [F_fqZ] for the GeRM_xde model.
#' @inheritParams F_fqZ
#' @return a [numeric] vector of length `nPatches`
#' @export
F_fqZ.GeRM_xde <- function(t, y, pars, s) {
  with(pars$MYZpar[[s]], f*q)*y[pars$ix$MYZ[[s]]$Z_ix]
}

#' @title Blood feeding rate of the infective mosquito population
#' @description Implements [F_fqM] for the GeRM_xde model.
#' @inheritParams F_fqM
#' @return a [numeric] vector of length `nPatches`
#' @export
F_fqM.GeRM_xde <- function(t, y, pars, s) {
  with(pars$MYZpar[[s]], f*q)*y[pars$ix$MYZ[[s]]$M_ix]
}


#' @title Number of eggs laid by adult mosquitoes
#' @description Implements [F_eggs] for the GeRM_xde model.
#' @inheritParams F_eggs
#' @return a [numeric] vector of length `nPatches`
#' @export
F_eggs.GeRM_xde <- function(t, y, pars, s) {
...
#' @description Implements [dMYZdt] for the GeRM_xde ODE model.
...
#' @title Derivatives for adult mosquitoes
#' @description Implements [dMYZdt] for the GeRM_xde DDE model.
#' @inheritParams dMYZdt
#' @return a [numeric] vector
...
make_MYZpar_GeRM_xde = function(nPatches, MYZopts=list(), EIPopts, calK,
  ...
  with(MYZopts,{
    ...
    if(solve_as == 'dde') class(MYZpar) <- c('GeRM_xde', 'GeRM_dde')
    if(solve_as == 'ode') class(MYZpar) <- c('GeRM_xde', 'GeRM_ode')
...
#' @title Setup the GeRM_xde model
#' @description Implements [setup_MYZinits] for the GeRM_xde model
#' @inheritParams setup_MYZinits
#' @return a [list] vector
#' @export
setup_MYZinits.GeRM_xde = function(pars, s, MYZopts=list()){
  pars$MYZinits[[s]] = with(pars$MYZpar[[s]], make_MYZinits_GeRM_dde(nPatches, Upsilon, MYZopts))
}


#' @title Make inits for GeRM_xde adult mosquito model
...
#' @description Implements [make_indices_MYZ] for the GeRM_xde model.
...
#' @title Parse the output of deSolve and return variables for the GeRM_xde model
#' @description Implements [parse_outputs_MYZ] for the GeRM_xde model
#' @inheritParams parse_outputs_MYZ
#' @return a [list]
#' @export
parse_outputs_MYZ.GeRM_xde <- function(outputs, pars, s) {with(pars$ix$MYZ[[s]],{
  time = outputs[,1]
  M = outputs[,M_ix+1]
  G = outputs[,G_ix+1]
  Y = outputs[,Y_ix+1]
...
#' @description Implements [get_inits_MYZ] for the GeRM_xde model.
...
#' @title Update the initial values for GeRM_xde adult mosquito model

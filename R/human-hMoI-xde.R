...
#' @export
F_X.hMoI_xde <- function(t, y, pars, i) {
  with(pars$ix$X[[i]],{
  m1 = y[m1_ix]
  m2 = y[m2_ix]
  H <- F_H(t, y, pars, i)
...
#' @return a [numeric] vector of length `nStrata`
#' @export
F_H.hMoI_xde <- function(t, y, pars, i) {
  pars$Hpar[[i]]$H
}

#' @title Compute the "true" prevalence of infection / parasite rate
#' @description Implements [F_pr] for the hMoI_xde model.
#' @inheritParams F_pr
#' @return a [numeric] vector of length `nStrata`
#' @export
F_pr.hMoI_xde<- function(varslist, pars, i) {
  pr = with(varslist$XH[[i]], 1-exp(-m1))
  return(pr)
}

#' @title Infection blocking pre-erythrocytic immunity
#' @description Implements [F_b] for the hMoI_xde model.
#' @inheritParams F_b
#' @return a [numeric] vector of length `nStrata`
#' @export
F_b.hMoI_xde <- function(y, pars,i) {
  with(pars$Xpar[[i]], b)
}
...
#' @export
dXdt.hMoI_xde <- function(t, y, pars, i) {
...
#' @title Setup Xinits.hMoI_xde
#' @description Implements [setup_Xinits] for the hMoI_xde model
#' @inheritParams setup_Xinits
#' @return a [list] vector
#' @export
setup_Xinits.hMoI_xde = function(pars, i, Xopts=list()){
  pars$Xinits[[i]] = make_Xinits_hMoI_xde(pars$Hpar[[i]]$nStrata, Xopts)
  return(pars)
}
...
#' @return none
#' @export
make_Xpar_hMoI_xde = function(nStrata, Xopts=list(),
...
#' @return none
#' @export
make_Xinits_hMoI_xde = function(nStrata, Xopts = list(), m10=2, m20=1){with(Xopts,{
...
#' @title Compute the HTC for the hMoI_xde model
#' @description Implements [HTC] for the hMoI_xde model with demography.
#' @inheritParams HTC
#' @return a [numeric] vector
#' @export
HTC.hMoI_xde <- function(pars, i) {
  with(pars$Xpar[[i]],
...
#' @export
make_indices_X.hMoI_xde <- function(pars, i) {with(pars,{
...
#' @return none
#' @export
make_inits_X_hMoI_xde <- function(pars, m10, m20) {
...
#' @return none
#' @export
update_inits_X.hMoI_xde <- function(pars, y0,i) {
  with(pars$ix$X[[i]],{
    m1 = y0[m1_ix]
    m2 = y0[m2_ix]
    pars$Xinits[[i]] = make_Xinits_hMoI_xde(pars$Hpar[[1]]$nStrata, m10=m1, m20=m2)
    return(pars)
})}
...
#' @return none
#' @export
get_inits_X.hMoI_xde <- function(pars, i){with(pars$Xinits[[i]],{
  c(m1, m2)
})}

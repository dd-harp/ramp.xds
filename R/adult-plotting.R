...
xds_plot_M = function(pars, s=1, clrs="darkblue", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  if(add_axes == TRUE) with(pars$outputs$orbits$MYZ[[s]],
                            plot(time, 0*time, type = "n", ylim = range(0,max(M)),
                                 ylab = "Mosquito Density", xlab = "Time"))

  xds_lines_M(pars$outputs$orbits$MYZ[[s]], pars, clrs, llty)
}
...
#'
#' @export
xds_lines_M = function(MYZ, pars, clrs="darkblue", llty=1){
  with(MYZ,{
...
xds_plot_YZ = function(pars, s=1, Yclrs = "purple", Zclrs="darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  if(add_axes == TRUE)
    with(pars$outputs$orbits$MYZ[[s]],
         plot(time, 0*time, type = "n", ylim = range(0,max(Y,Z)),
              ylab = "Mosquito Density", xlab = "Time"))

  xds_lines_YZ(pars$outputs$orbits$MYZ[[s]], pars, Yclrs, Zclrs, llty)
}
...
#'
#' @export
xds_lines_YZ = function(MYZ, pars, Yclrs="purple", Zclrs = "darkred", llty=1){
  with(MYZ,{
    if(pars$nPatches==1){
...
xds_plot_YZ_fracs = function(pars, s=1, Yclrs = "purple", Zclrs = "darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xds_lines_YZ_fracs(pars$outputs$orbits$MYZ[[s]], pars, Yclrs, Zclrs, llty)
}
...
#'
#' @export
xds_lines_YZ_fracs = function(MYZ, pars, Yclrs="purple", Zclrs="darkred", llty=1){
  with(MYZ,{
    if(pars$nPatches==1) {
...
xds_plot_Z_fracs = function(pars, s=1, Zclrs = "darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  if(add_axes == TRUE)
    with(pars$outputs$orbits$MYZ[[s]],
         plot(time, 0*time, type = "n", ylim = range(0,pars$outputs$orbits$MYZ[[s]]$z),
              ylab = "Fraction Infected", xlab = "Time"))

  xds_lines_Z_fracs(pars$outputs$orbits$MYZ[[s]], pars,  Zclrs, llty)
}
...
#'
#' @export
xds_lines_Z_fracs = function(MYZ, pars, Zclrs="darkred", llty=1){
  with(MYZ,{
    if(pars$nPatches==1) {
      lines(time, z, col=Zclrs, lty = llty[1])
    }

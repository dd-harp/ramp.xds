...
xds_plot_M = function(pars, s=1, clrs="darkblue", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xds_lines_M(vars$MYZ[[s]], pars, clrs, llty)
}
...
xds_plot_YZ = function(pars, s=1, Yclrs = "purple", Zclrs="darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xds_lines_YZ(vars$MYZ[[s]], pars, Yclrs, Zclrs, llty)
}
...
xds_plot_YZ_fracs = function(pars, s=1, Yclrs = "purple", Zclrs = "darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xds_lines_YZ_fracs(vars$MYZ[[s]], pars, Yclrs, Zclrs, llty)
}
...
xds_plot_Z_fracs = function(pars, s=1, Zclrs = "darkred", llty=1, stable=FALSE, add_axes=TRUE){
  ...
  xds_lines_Z_fracs(vars$MZ[[s]], pars,  Zclrs, llty)
}

#' Add lines for the fraction of infected and infective mosquitoes
#'
#' @param MZ a list the output of `ramp.xde::parse_deout()`
#' @param pars a list that defines an `ramp.xde` model (*e.g.*,  generated by `xde_setup()`)
#' @param Zclrs a vector of colors for infective mosquitoes
#' @param llty an integer (or integers) to set the `lty` for plotting
#'
#' @export
xds_lines_Z_fracs = function(MZ, pars, Zclrs="darkred", llty=1){
  with(MZ,{
    if(pars$nPatches==1) {
      lines(time, z, col=Zclrs, lty = llty[1])
    }
